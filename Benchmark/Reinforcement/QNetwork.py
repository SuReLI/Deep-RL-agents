...
class QNetwork:
    ...
    def __init__(self, sess, gui, buffer):
        """
    ...
    def run(self):
        ...
        with self.sess.as_default(), self.sess.graph.as_default():
            ...
            while not self.gui.STOP:
                ...
                if self.total_eps % Settings.PERF_FREQ == 1:
                    print("PERF : %i learning round in %.3fs" %
                          (Settings.PERF_FREQ, time.time() - batch_time))
                    time_gd.append(time.time() - batch_time)
                    batch_time = time.time()
        print('\nAverage time of a gradient descent : %.4fs' % (sum(time_gd) / len(time_gd)))
