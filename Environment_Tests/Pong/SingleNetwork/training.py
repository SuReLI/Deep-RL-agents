...
def policy_loss(y_true, y_pred):  # policy loss
    return -K.sum(K.log(K.abs(y_true * y_pred + (1 - y_true) * (1 - y_pred)) + 1e-5), axis=-1)
...
def buildmodel():
    ...
    h0 = Convolution2D(32, kernel_size=(8, 8), strides=(4, 4), activation='relu',
                       kernel_initializer='random_uniform', bias_initializer='random_uniform')(S)
    h1 = Convolution2D(64, kernel_size=(4, 4), strides=(2, 2), activation='relu',
    ...
    P = Dense(action_size, name='o_P', activation='softmax',
...
for i in range(THREADS):
    game_state.append(gym.make("PongDeterministic-v4"))
...
class actorthread(threading.Thread):
    ...
    def runprocess(self):
        ...
        while episode_step < MAX_STEP and not done:
            ...
            state, reward, done, _ = game_state[self.thread_id].step(action)
            state = preprocess(state)

            # if self.thread_id == 1:
                # game_state[self.thread_id].render()
